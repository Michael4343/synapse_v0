...
serve(async (req) => {
  ...
  try {
    ...
    const baseItemsPerCategory = Math.floor(totalItems / enabledCategoryCount)
    const remainder = totalItems % enabledCategoryCount

    console.log('Filter preferences extracted:')
    console.log('- enabledCategories:', enabledCategories)
    console.log('- totalItems:', totalItems, '(from preferences?.itemsPerCategory:', preferences?.itemsPerCategory, ')')
    ...
    // Helper function to get items count for each category (distributes remainder)
    let categoryIndex = 0
    const getItemsForCategory = () => {
      const items = baseItemsPerCategory + (categoryIndex < remainder ? 1 : 0)
      categoryIndex++
      return items
    }
